...
router.get("/", auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (latestRecording) {
            try {
                const events = JSON.parse(latestRecording.events || "[]");
        ...
        res.json({
            ...
            data: {
                ...
                generatedScript: latestRecording
                    ? await (0, playwrightGenerator_1.generatePlaywrightCode)(JSON.parse(latestRecording.events || "[]"), latestRecording.id)
                    : "// No script generated"
            },
        });
    }
