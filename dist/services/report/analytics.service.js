...
async function generateAnalytics(recordingId) {
    try {
        ...
        const totalSteps = testRuns.reduce((sum, run) => sum + run.steps.length, 0);
        const passedSteps = testRuns.reduce((sum, run) => {
            const passed = run.steps.filter((s) => s.success).length;
            return sum + passed;
        }, 0);
        const avgDuration = testRuns.reduce((sum, run) => {
